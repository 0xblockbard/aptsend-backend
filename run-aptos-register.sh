...
echo "Executing Aptos registration script with data: $REQUEST_DATA"
...
timeout $TIMEOUT_DURATION "$PROJECT_ROOT/node_modules/.bin/tsx" \
  "$PROJECT_ROOT/resources/js/aptos/register_user.ts" "$REQUEST_DATA"
...
if [ $EXIT_CODE -eq 124 ]; then
  ...
  echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] ERROR: Command timed out after $TIMEOUT_DURATION seconds" >> "$PROJECT_ROOT/storage/logs/aptos/run-register-error.log"
